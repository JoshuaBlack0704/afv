...
fn main() -> ! {
    ...
    let pan = StepperMotor::new(pins.d5.into_output(), pins.d7.into_output(), None, None, 200, Some(16), 1000, false);
    let tilt = StepperMotor::new(pins.d3.into_output(), pins.d2.into_output(), None, None, 200, Some(16), 1000, true);

    let mut turret = Turret::new(pan, tilt, FLIR_TURRET_PORT, SocketBlock::SOCKET0, &mut spi, &mut cs, &mut serial);

    loop{
        turret.process(&mut spi, &mut cs, &mut serial);
    }
}
