...
use crate::timer::{Clock, ComMode, Timer1, Waveform};

const TOPVALUE: u16 = 5000;
...
impl Servo {
    pub fn new(
        ...
        timer.load_icr1(TOPVALUE);
        // Set out pin outputs
        if use_pb1 {
            timer.set_coma(ComMode::FASTPWMNONINV);
        } else {
            timer.set_coma(ComMode::DISCONNECTED);
        }
        if use_pb2 {
            timer.set_comb(ComMode::FASTPWMNONINV);
        } else {
            timer.set_comb(ComMode::DISCONNECTED);
        }
        ...
        // Start timer
        // NOTE: If the prescaler changes, you must change TOPVALUE
    ...
    pub fn set_pb1_angle(&self, angle: f32) {
        ...
        let value = TOPVALUE as f32 * (duty / 100.0);
