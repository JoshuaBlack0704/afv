...
pub trait StepperOps{
    ...
    fn to_step(&mut self, step: i32, exit_on_max_steps: bool, serial: &mut Usart0<MHz16>) -> Result<i32, StepperOpsError>;
...
impl<S: OutputPin, D: OutputPin> StepperMotor<S,D>{
    pub fn new(step_pin: S, dir_pin: D, max_clockwise: i32, min_clockwise: i32, microsteps: Option<u32>, step_time_us: u32, microstep_time: u32, inverted: bool) -> Self{
        Self{
            step: step_pin,
            dir: dir_pin,
            microsteps,
...
impl<S: OutputPin, D: OutputPin> StepperOps for StepperMotor<S,D>{
    ...
    fn to_step(&mut self, step: i32, exit_on_step_limit: bool, serial: &mut Usart0<MHz16>) -> Result<i32, StepperOpsError> {
        let _ = ufmt::uwriteln!(serial, "Stepper going to step {} from step {}", step, self.current_step());
