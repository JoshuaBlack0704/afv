use arduino_hal::{hal::usart::Usart0, clock::MHz16, I2c, prelude::{_embedded_hal_blocking_i2c_WriteRead, _embedded_hal_blocking_i2c_Write}};
...
impl GarminLidarV3{
    ...
    pub fn new(address: Option<u8>, serial: &mut Usart0<MHz16>) -> GarminLidarV3 {
        ...
        if let Some(_) = address{
            let _ = ufmt::uwriteln!(serial, "Configure new address for garmin lidar not implemented");
        }
    ...
    // fn read<const N:usize>(&self, register: ControlRegister, i2c: &mut I2c) -> [u8; N] {
    //     let mut data = [0u8; N];
    //     let cmd = [register.into()];
    //     let _ = i2c.write_read(self.address, &cmd, data.as_mut_slice());
    //     data
    // } 
    // fn write<const N:usize>(&self, register: ControlRegister, data: &[u8; N], i2c: &mut I2c){
    //     let cmd: [u8; 1 + N] = [];
    //     let _ = i2c.write(self.address, cmd.concat(data));
