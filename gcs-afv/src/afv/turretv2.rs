...
impl Link{
    ...
    async fn update(self: Arc<Self>){
        ...
        loop{
            sleep(wait).await;
            self.com.send(AfvMessage::Turret2(Turret2Msg::GetAltitude(self.code))).await;
            self.com.send(AfvMessage::Turret2(Turret2Msg::GetAzimuth(self.code))).await;
        }
    }
}
...
impl Controller for Link{
    ...
    async fn set_altitude(&self, altitude: f32) {
        self.com.send(AfvMessage::Turret2(Turret2Msg::SetAltitude(self.code, altitude))).await;
    }

    async fn set_azimuth(&self, azimuth: f32) {
        self.com.send(AfvMessage::Turret2(Turret2Msg::SetAzimuth(self.code, azimuth))).await;
    }
}
...
impl ComEngineService<AfvMessage> for Link{
    async fn notify(self: Arc<Self>, com: Arc<ComEngine<AfvMessage>>, msg: AfvMessage){
        if let AfvMessage::Turret2(m) = msg{
            match m{
                Turret2Msg::SetAltitude(code, alt) => {
                    if code == self.code{
                        *self.altitude.write().await = alt;
                    }
                },
                Turret2Msg::SetAzimuth(code, azi) => {
                    if code == self.code{
...
impl ComEngineService<AfvMessage> for Simulator{
    async fn notify(self: Arc<Self>, com: Arc<ComEngine<AfvMessage>>, msg: AfvMessage){
        if let AfvMessage::Turret2(m) = msg{
            match m{
                Turret2Msg::GetAltitude(code) => {
                    if let Some(c) = &self.com{
                        c.send(AfvMessage::Turret2(Turret2Msg::SetAltitude(self.code, self.get_altitude().await))).await;
                    }
                },
                Turret2Msg::GetAzimuth(code) => {
                    if let Some(c) = &self.com{
                        c.send(AfvMessage::Turret2(Turret2Msg::SetAzimuth(self.code, self.get_azimuth().await))).await;
                    }
                },
                Turret2Msg::SetAltitude(code, alt) => {
                    if code == self.code{
